...
export default function ProfilePage() {
  ...
  const [dragActive, setDragActive] = useState<boolean[]>([false, false, false, false]);
  ...
  const handleDrag = (e: React.DragEvent, slotIndex: number) => {
    ...
    if (e.type === "dragenter" || e.type === "dragover") {
      ...
      newDragActive[slotIndex] = true;
      setDragActive(newDragActive);
    } else if (e.type === "dragleave") {
      const newDragActive = [...dragActive];
      newDragActive[slotIndex] = false;
      setDragActive(newDragActive);
    }
  };
  ...
  const handleDrop = (e: React.DragEvent, slotIndex: number) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files[0]) {
      handleFileUpload(e.dataTransfer.files[0], slotIndex);
    }
  };
    ...
    try {
      ...
      const { error } = await supabase.auth.updateUser({
        data: {
          ...
          bio: formData.bio,
          // Keep existing profile pictures if they exist
