...
const FeaturedSubleases = ({ city, uniFilter }: { city?: string | null; uniFilter?: UniFilterProps }) => {
  const router = useRouter();
  const [properties, setProperties] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const { language } = useLanguage();
  const t = translations[language].featuredSubleases;
  const title = city
    ? language === "ko" ? `${city} 근처 전대차` : `Subleases near ${city}`
    : t.title;
  ...
  useEffect(() => {
    const fetchProperties = async () => {
      try {
        ...
        console.error("Failed to fetch featured subleases:", err);
      } finally {
        setLoading(false);
      }
    };
      ...
      <section className="py-12 px-4 bg-white">
        <div className="container mx-auto">
          <div className="flex justify-between items-center mb-8">
            ...
            <Button variant="outline" onClick={() => router.push("/search?type=sublet")}>{t.viewAll}</Button>
          </div>
...
export default FeaturedSubleases;
